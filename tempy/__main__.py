...
def main():
    ...
    parser.add_argument(
        "-t",
        "--translate",
        help="Translates a .html file into a Tempy .py file.",
        action="store_true",
    )

    actions, rem_args = parser.parse_known_args()
    if actions.translate:
        translate_group = parser.add_argument_group(
            "Translation of html into Tempy python files"
        )
        ...
        translate_group.add_argument(
            "-p",
            "--pretty",
            help="Makes the output file indented.",
            action="store_true",
        )
